...
if __name__ == "__main__":
    ...
    if args.run_test:
        ...
        data = pd.read_csv(os.path.join(data_path, "CO2_exp.txt"), parse_dates=["date"])
        ...
        data = pd.read_csv("./data/CH4_exp_32.txt",sep=" ",names=['date','time','CH4','CH4_T','T','P','RH'],low_memory=False)
        data=data.dropna(axis=0,how='any')
        Xdate = pd.to_datetime(data['date'],format='%Y-%m-%d')
        Xtime = pd.to_datetime(data['time'],format='%H:%M:%S')
        ...
        P = data['P']
        RH = data['RH']
        CH4= pd.to_numeric(CH4,errors='coerce')
        CH4_T= pd.to_numeric(CH4_T,errors='coerce')
        T= pd.to_numeric(T,errors='coerce')
        P= pd.to_numeric(P,errors='coerce')
        RH= pd.to_numeric(RH,errors='coerce')
        Xyear = pd.to_numeric(Xdate.apply(lambda x: x.year),errors='coerce')
        Xdoy = pd.to_numeric(Xdate.apply(lambda x: x.dayofyear),errors='coerce')
        Xh = pd.to_numeric(Xtime.apply(lambda x: x.hour),errors='coerce')
        Xm = pd.to_numeric(Xtime.apply(lambda x: x.minute),errors='coerce')
        Xs = pd.to_numeric(Xtime.apply(lambda x: x.second),errors='coerce')


        #X = np.c_[np.asarray(hours), np.asarray(dows)]
        X = np.c_[np.asarray(Xyear),np.asarray(Xdoy),np.asarray(Xh),np.asarray(Xm),np.asarray(Xs),np.asarray(CH4),np.asarray(T),np.asarray(P),np.asarray(RH)]
        #X = np.c_[np.asarray(CH4),np.asarray(T),np.asarray(P),np.asarray(RH)]
        num_features = X.shape[1]
        num_periods = len(data)
        X = np.asarray(X).reshape((-1, num_periods, num_features))
        y = np.asarray(data["CH4_T"]).reshape((-1, num_periods))
        # X = np.tile(X, (10, 1, 1))
        # y = np.tile(y, (10, 1))
